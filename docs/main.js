jQuery(function () {
	$("button[type=submit]").bind("click", function () {
		...
		$.ajax({
			...
			beforeSend: function () {
				if ($("[name=nombre]", $form).val() === "" || $("[name=apellido]", $form).val() === "" ||
					$("[name=email]", $form).val() === "" || opcionesParticipacion.length == 0) {
						$("button[type=submit]", $form).prop('disabled', true);
						$("button[type=submit]", $form).html('Asegúrese de llenar todo el formulario');
						setTimeout(function () {
